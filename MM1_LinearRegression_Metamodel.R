...
lm_model <- lm(avg_wait_time ~ service_time, data = results)
predicted <- predict(lm_model, newdata = results)
mse <- mean((results$avg_wait_time - predicted)^2)
cat("Mean Squared Error (MSE):", round(mse, 4), "\n")
summary(lm_model)


# Plot Results
ggplot(results, aes(x = service_time, y = avg_wait_time)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Linear Regression Metamodel",
       x = "Mean Service Time",
       y = "Average Wait Time")
